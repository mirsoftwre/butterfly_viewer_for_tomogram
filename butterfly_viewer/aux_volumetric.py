...
class VolumetricImageHandler:
    ...
    def _analyze_file(self):
        ...
        """
        try:
            self._memmap = tifffile.memmap(self.filepath)
            if self._memmap.ndim == 3:
                arr = self._memmap
            else:
                arr = None
            ...
            self._memmap = None
            arr = None
            print(f"memmap failed: {e}")

        if arr is not None:
            self.use_tifffile = True
            self.total_slices = arr.shape[0]
            self.bit_depth = arr.dtype.itemsize * 8
            self.is_float = np.issubdtype(arr.dtype, np.floating)
        ...
        try:
            with tifffile.TiffFile(self.filepath) as tif:
                ...
                for page in tif.pages:
                    arr = page.asarray()
                    slice_min = np.min(arr)
                    slice_max = np.max(arr)
